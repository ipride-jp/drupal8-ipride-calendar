...
class SimpleCalDAVClient {
  ...
  function findCalendar($calendar_id) {
    return $this->client->GetCalendarDetailsByID($calendar_id);
  }
  ...
  function deleteCalendar($calendar_id) {
    ...
    $url = sprintf('%scalendars/%s/%s', $this->base_url, $this->user, $calendar_id);

    // Do the deletion
    $this->client->DoDELETERequest($url);
  ...
  function updateCalendar($calendar_id, $display_name, $color, $order = NULL) {
    if (!isset($this->client)) {
      throw new Exception('No connection. Try connect().');
    }
    ...
    $order_str = '';
    if ($order !== NULL) {
      $order_str = "<A:calendar-order>$order</A:calendar-order>";
    }
...
EOXML;
    ...
    $url = sprintf('%scalendars/%s/%s', $this->base_url, $this->user, $calendar_id);
    $this->client->DoXMLRequest('PROPPATCH', $xml, $url);
    if ( $this->client->GetHttpResultCode() != '200' and $this->client->GetHttpResultCode() != '207' )
    {
      throw new CalDAVException('Recieved unknown HTTP status', $this->client);
    }
  }

  function getEvent($calendar_id, $event_id) {
    $obj = $this->client->GetCalendarObjectById($calendar_id, $event_id);
    if (!$obj) {
      return NULL;
    }

    return new CalDAVObject($obj['href'], $obj['data'], $obj['etag']);
  }
  ...
  function createCalendar($calendar_id, $display_name, $color) {
...
EOXML;
    $url = sprintf('%scalendars/%s/%s', $this->base_url, $this->user, $calendar_id);
    $this->client->DoXMLRequest('MKCALENDAR', $xml, $url);

    if ($this->client->GetHttpResultCode() != '200'
      and $this->client->GetHttpResultCode() != '201'
    ) {
      throw new CalDAVException('Recieved unknown HTTP status', $this->client);
    }
  }

  function shareCalendar($calendar_id, $display_name, $attendees) {
    $old_attendees = [];
    if ($calendar = $this->findCalendar($calendar_id)) {
      $old_attendees = $calendar->getAttendees();
    }

    $attendee_mails = array_map(function($a) { return $a['mail']; }, $attendees);
    $old_attendee_mails = array_map(function($a) { return $a['mail']; }, $old_attendees);
    $mails_to_remove = array_diff($old_attendee_mails, $attendee_mails);
    ...
    foreach ($attendees as $attendee) {
      ...
      $set_xml = <<<EOXML
  <C:set>
    <A:href xmlns:A="DAV:">mailto:$mail</A:href>
    <C:common-name/>
    <C:summary>$display_name</C:summary>
    <C:$permission/>
  </C:set>
EOXML;
    ...
    $remove_xmls = '';
    foreach ($mails_to_remove as $mail_to_remove) {
      $remove_xml = <<<EOXML
  <C:remove>
    <A:href xmlns:A="DAV:">mailto:$mail_to_remove</A:href>
  </C:remove>
EOXML;

      $remove_xmls .= $remove_xml . "\n";
    }
...
EOXML;
    ...
    $url = sprintf('%scalendars/%s/%s', $this->base_url, $this->user, $calendar_id);
    $this->client->DoXMLRequest('POST', $xml, $url);
  ...
  function change ( $href, $new_data, $etag )
  {
    ...
    if ( $this->client->GetHttpResultCode() != '204' && $this->client->GetHttpResultCode() != '200' )
    {
      throw new CalDAVException(
        'Recieved unknown HTTP status. ' . $this->client->GetXmlResponse(),
        $this->client
      );
    }
