...
class CalDAVCalendar {
  ...
  function setOrganizer($organizer_url) {
    if (preg_match('/\/principals\/(.*)/', $organizer_url, $matches)) {
      $this->organizer = NULL;
      $user = user_load_by_name($matches[1]);
      if ($user) {
        $this->organizer = $user->getEmail();
      }
    }
  }

  function addAttendee($attendee_mailto, $attendee_permission) {
    if (preg_match('/mailto:(.*)/', $attendee_mailto, $matches)) {
      $this->attendees[] = [
        'mail' => $matches[1],
        'permission' => $attendee_permission,
      ];
    }
  }
}
