...
*/
  ...
  function DoPUTRequest( $url, $icalendar, $etag = null ) {
      ...
      if (preg_match( '{^ETag:\s+"([^"]*)"\s*$}im', $this->httpResponseHeaders, $matches )) {
        $etag = $matches[1];
      } else if ( preg_match( '{^ETag:\s+([^\s]*)\s*$}im', $this->httpResponseHeaders, $matches ) ) {
        $etag = $matches[1];
      }
  ...
  function GetCalendarDetailsByID($id) {
    $calendar_home = $this->FindCalendarHome(true);
    $url = $this->first_url_part . $calendar_home[0] . $id;

    $calendar_objs = $this->GetCalendarDetailsByURL($url);
    if (key_exists($id, $calendar_objs)) {
      return $calendar_objs[$id];
    }

    return NULL;
  }

  function GetCalendarObjectById($calendar_id, $event_id) {
    $calendar_home = $this->FindCalendarHome(true);
    $url = $this->first_url_part . $calendar_home[0] . $calendar_id . '/' . $event_id . '.ics';

    return $this->GetCalendarObjectByUrl($url);
  }

  private function GetCalendarObjectByUrl($url) {
    $entry = $this->GetEntryByHref($url);
    if (!$entry) {
      return NULL;
    }
                  ...
                  switch( $v['tag'] ) {
                      ...
                      case 'http://apple.com/ns/ical/:calendar-order':
                          $calendar->setOrder((isset($v['value']) ?
                              $v['value'] : '-'));
                          break;
                      case 'http://calendarserver.org/ns/:organizer':
                          $in_organizer_block = $v['type'] == 'open';
                          break;
                      case 'http://calendarserver.org/ns/:user':
                          $in_user_block = $v['type'] == 'open';
                          if ($v['type'] == 'open') {
                            $attendee_mail = '';
                            $attendee_permission = '';
                          } else {
                            $calendar->addAttendee($attendee_mail, $attendee_permission);
                          }
                          break;
                      case 'DAV::href':
                          if ($in_organizer_block) {
                            $calendar->setOrganizer($v['value']);
                          } elseif ($in_user_block) {
                            $attendee_mail = $v['value'];
                          }
                          break;
                      case 'http://calendarserver.org/ns/:read-write':
                          $attendee_permission = 'read-write';
                          break;
                      case 'http://calendarserver.org/ns/:read':
                          $attendee_permission = 'read';
                          break;
                  }
              }
