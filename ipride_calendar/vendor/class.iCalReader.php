    ...
    public function addCalendarComponentWithKeyAndValue($component, $keyword, $value)
    {
        ...
        switch ($component) {
            ...
            case 'VALARM':
                ...
                $event =& $this->cal['VEVENT'][$this->event_count - 1];
                if (!isset($event['ALARM'][$this->alarm_count - 1][$keyword . '_array'])) {
                  $event['ALARM'][$this->alarm_count - 1][$keyword . '_array'] = array();
                }

                if (is_array($value)) {
                  $event['ALARM'][$this->alarm_count - 1][$keyword . '_array'] = $value;
                } else {
                  $event['ALARM'][$this->alarm_count - 1][$keyword] = $value;
                }
                break;
            case 'VEVENT':
